...
var cube_coords: Vector3 = Vector3(0, 0, 0) :
	get:
		return cube_coords
	set(mod_value):
		if abs(mod_value.x + mod_value.y + mod_value.z) > 0.0001:
			print("WARNING: Invalid cube coordinates for hex (x+y+z!=0): ", mod_value)
			return
		cube_coords = round_coords(mod_value)
# but other coord systems can be used
var axial_coords: Vector2 :
	get:
		return Vector2(cube_coords.x, cube_coords.y)
	set(mod_value):
		set_cube_coords(axial_to_cube_coords(mod_value))
var offset_coords: Vector2 :
	get:
		var x: int = int(cube_coords.x)
		var y: int = int(cube_coords.y)
		var off_y: int = y + (x - (x & 1)) / 2
		return Vector2(x, off_y)
	set(mod_value):
		var x: int = int(mod_value.x)
		var y: int = int(mod_value.y)
		var cube_y: int = y - (x - (x & 1)) / 2
...
func line_to(target):
	...
	target = obj_to_coords(target)
	# End of our lerp is nudged so it never lands exactly checked an edge
