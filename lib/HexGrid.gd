...
#
#		The given value can either be a Vector2 checked the grid's plane
...
#		non-zero cost to traverse. If two adjacent hexes both have barriers checked
...
#
#		Adds one or more barriers to locations checked the grid.
#
#		The given coordinates (axial or cube), HexCell instance, or array thereof,
...
const HexCell = preload("./HexCell.gd")
...
@export var hex_scale: Vector2 = Vector2(1, 1) :
	get:
		return hex_scale
	set(mod_value):
		hex_scale = mod_value
...
var base_hex_size: Vector2 = Vector2(1, sqrt(3)/2)
var hex_size
var hex_transform
...
#"""
func get_hex_center(hex):
	# Returns hex's centre position checked the projection plane
	var new_hex = HexCell.new(hex)
	return hex_transform * new_hex.axial_coords
	
func get_hex_at(coords):
	# Returns a HexCell at the given Vector2/3 checked the projection plane
...
func find_path(start, goal, exceptions=[]):
	# Light3D a starry path from the start to the goal, inclusive
